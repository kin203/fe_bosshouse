        ...
        <div className="flex bg-white rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-4xl">
          ...
          <h2 className="text-2xl font-bold  text-center xl:block hidden">BossHouse</h2>
            ...
            <div className=" my-3 flex justify-center items-center">
              <GoogleLogin
                onSuccess={(credentialResponse) => {
                  const decoded: { email: string; aud: string } = jwtDecode(
                    credentialResponse?.credential
                  );
                  (async () => {
                    const getOneUser = await getByEmail({
                      email: decoded?.email,
                    });
                    if (getOneUser?.data?.length == 0) {
                      const a: any = await signUp({
                        email: decoded?.email,
                        password: decoded?.aud?.slice(0, 7) + "test",
                        confirmPassword: decoded?.aud?.slice(0, 7) + "test",
                      });
                      ...
                      const isChecked = await checkPassword(
                        decoded?.aud?.slice(0, 7) + "test",
                        getOneUser.data[0].password
                      );
                      ...
                      if (isChecked) {
                        ...
                        Swal.fire({
                          title:
                            "Tài khoản đăng ký bằng email này đã tồn tại trên hệ thống!",
                          icon: "error",
                        });
                      }
                    }
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <p className="text-xs text-center text-gray-700 uppercase">
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <Link
                  to={"/signin"}
                  className="text-xs text-gray-800 uppercase xl:hidden block"
                >
                  hoặc Đăng nhập
                </Link>
                <span className="border-b w-1/5 md:w-1/4"></span>
